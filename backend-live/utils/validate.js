...
const URL_RE =
  /^(https?:\/\/)((([\w-]+\.)+[\w-]{2,})|localhost|(\d{1,3}\.){3}\d{1,3})(:\d+)?(\/[\w\-./?%&=+#~:@]*)?$/i;
...
export const validateUrl = (value, label = "URL", { required = false } = {}) => {
  ...
  // Uploaded files are stored and returned as relative paths (e.g.
  // "/api/uploads/xxx.jpg"), not full http(s) URLs — accept those too,
  // in addition to full URLs.
  if (!URL_RE.test(sanitized) && !sanitized.startsWith("/")) {
