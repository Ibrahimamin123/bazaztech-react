...
const CmsManager = ({
  ...
  const validateForm = () => {
    ...
    fields.forEach((field) => {
      ...
      // Uploaded image URLs may include localhost, ports, or proxy hosts —
      // accept any http(s) URL rather than a strict public-domain pattern.
      if (field.type === "upload" && value) {
        const trimmed = String(value).trim();
        const ok = /^https?:\/\/.+/i.test(trimmed) || trimmed.startsWith("/");
        checks.push({
          field: field.name,
          message: ok ? "" : `Enter a valid ${field.label.toLowerCase()} URL.`,
        });
      }
  ...
  const handleSubmit = async (e) => {
    ...
    // Uploaded media always wins over legacy star/icon flags.
    if (payload.image) {
      payload.useStars = false;
    }
                      ...
                      {fields.slice(0, 3).map((field) => (
                        <td key={field.name}>
                          {field.type === "image" || field.type === "upload" ? (
                            <img
                              src={resolveImageSrc(item[field.name]) || "https://via.placeholder.com/50"}
                              alt=""
                            ...
                            <img
                              src={resolveImageSrc((item[field.name] || [])[0]) || "https://via.placeholder.com/50"}
                              alt=""
                              className="table-thumb"
                              onError={(e) => {
                                e.currentTarget.onerror = null;
                          ...
                          {field.type === "textarea" ? (
                            ...
                            <>
                              ...
                              <small className="text-muted d-block mt-1" style={{ whiteSpace: "pre-line" }}>
                                {field.hint || "Maximum File Size: 1 MB each\nRecommended Dimensions: 1200 × 800 px\nWidth: 1200 px\nHeight: 800 px"}
                              </small>
                            ...
                            <>
                              ...
                              />
                              <small className="text-muted d-block mt-1" style={{ whiteSpace: "pre-line" }}>
                                {field.hint || "Maximum File Size: 1 MB\nRecommended Dimensions: 1200 × 800 px\nWidth: 1200 px\nHeight: 800 px"}
                              </small>
                              {form[field.name] && (
