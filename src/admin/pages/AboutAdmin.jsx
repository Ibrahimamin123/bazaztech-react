...
const AboutAdmin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const nextErrors = collectErrors([
      ...
      { field: "missionTitle", message: validateRequired(form.missionTitle, "Mission heading") },
      { field: "missionDescription", message: validateRequired(form.missionDescription, "Mission paragraph") },
      { field: "missionDescription", message: validateMaxLength(form.missionDescription, 1000, "Mission paragraph") },
      { field: "visionTitle", message: validateRequired(form.visionTitle, "Vision heading") },
      { field: "visionDescription", message: validateRequired(form.visionDescription, "Vision paragraph") },
      { field: "visionDescription", message: validateMaxLength(form.visionDescription, 1000, "Vision paragraph") },
    ...
    try {
      setSaving(true);
      // Mission / Vision are text-only — clear any legacy image fields.
  ...
  const renderImageField = (fieldName, label) => (
    <div className="col-md-4">
      ...
      {errors[fieldName] && <div className="invalid-feedback d-block">{errors[fieldName]}</div>}
      {form[fieldName] && (
        <img src={resolveImageSrc(form[fieldName])} alt={`${label} preview`} className="table-thumb mt-2" />
      )}
    </div>
  );
    ...
    <AdminLayout title="About Us">
      <div className="dashboard-content">
        <p className="text-muted mb-4">
          Manage About Us content. Mission &amp; Vision appear at the top (heading + paragraph only).
          Founder &amp; CEO appear below with photos.
        </p>
        ...
        <form className="table-card" onSubmit={handleSubmit} noValidate>
          ...
          <div className="row g-3">
            <div className="col-md-12">
              ...
              <input
                className={`form-control ${errors.missionTitle ? "is-invalid" : ""}`}
                name="missionTitle"
                value={form.missionTitle}
                onChange={handleChange}
                placeholder="Our Mission"
              />
              {errors.missionTitle && <div className="invalid-feedback d-block">{errors.missionTitle}</div>}
            </div>
            <div className="col-12">
              <label className="form-label">Paragraph</label>
              <textarea
                className={`form-control ${errors.missionDescription ? "is-invalid" : ""}`}
                rows="4"
              ...
              />
              {errors.missionDescription && <div className="invalid-feedback d-block">{errors.missionDescription}</div>}
            </div>
          </div>
          ...
          <div className="row g-3">
            <div className="col-md-12">
              <label className="form-label">Heading</label>
              <input
                className={`form-control ${errors.visionTitle ? "is-invalid" : ""}`}
                name="visionTitle"
                value={form.visionTitle}
                onChange={handleChange}
                placeholder="Our Vision"
              />
              {errors.visionTitle && <div className="invalid-feedback d-block">{errors.visionTitle}</div>}
            </div>
            <div className="col-12">
              <label className="form-label">Paragraph</label>
              <textarea
                className={`form-control ${errors.visionDescription ? "is-invalid" : ""}`}
                rows="4"
              ...
              />
              {errors.visionDescription && <div className="invalid-feedback d-block">{errors.visionDescription}</div>}
            </div>
          </div>
          ...
          <div className="row g-3">
            {renderImageField("founderImage", "Founder Image")}
            <div className="col-md-4">
          ...
          <div className="row g-3">
            {renderImageField("ceoImage", "CEO Image")}
            <div className="col-md-4">
